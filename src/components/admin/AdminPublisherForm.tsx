...
export function AdminPublisherForm({
  ...
  const hasPhoto =
    !!publisher?.photo && !publisher.photo.includes("placehold.co");
  ...
  return (
    <form action={formAction} className="space-y-5" noValidate>
      ...
      <FormField
        label="Designation"
        name="designation"
        defaultValue={publisher?.designation ?? ""}
        error={f.designation}
        hint='Shown under the name, e.g. "Publisher & Founder"'
      />
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        <FormField
          ...
          name="display_order"
          type="number"
          defaultValue={String(publisher?.display_order ?? 0)}
          error={f.display_order}
          hint="Lower shows first. The homepage shows the lowest-ordered active publisher."
        />
        <div className="flex items-center gap-3 pt-7">
          <input
            ...
            name="active"
            type="checkbox"
            defaultChecked={publisher ? publisher.active : true}
